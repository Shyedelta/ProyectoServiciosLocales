        ...
        <>
            <div className="px-10 mt-10 gap-3 h-max p-3 w-full grid grid-rows-1 lg:grid-cols-3 place-items-center">
                ...
                <div className='w-full overflow-hidden p-5 gap-5 rounded-xl bg-blue-400 grid grid-cols-2 content-between'>
                    ...
                    {user && user.email !== empresa?.email && (
                        <button
                            onClick={user !== null ?
                                () => setOpenModalMsg(true) :
                                () => setToast(true)
                            }
                            className={`justify-self-end w-20 inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white ${user !== null ? 'bg-blue-700 hover:bg-blue-800 cursor-pointer' : 'bg-gray-500 cursor-not-allowed'} rounded-lg focus:ring-4 focus:outline-none focus:ring-blue-300`}
                        >
