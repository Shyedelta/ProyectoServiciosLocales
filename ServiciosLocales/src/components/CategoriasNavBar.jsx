...
function CategoriasNavBar({ setOpenMenu }) {
  return (
    <div className=" shadow-2xl absolute top-16 right-0  w-screen overflow-auto min-h-[max-content] scroll-smooth ">
      <div onMouseLeave={() => { setOpenMenu(false) }} className="sticky z-50 w-full p-1 bg-white border-t border-b border-gray-200 ">
        <div className="max-w-[70%] h-full py-2 mx-auto font-medium flex justify-center align-middle">
          {otros[2].map((categoria, index) => (
            <div key={index} className='mx-auto '>
              {categoria.icon && <Link to={"/servicios/" + categoria.nombre}>
                <button type="button" className="inline-flex flex-col items-center justify-center px-4 hover:bg-gray-50 group">
          ...
          <div className='mx-auto '>
            <Link to={"/servicios"}> 
              <div className='text-gray-600 '>
                <button type="button" className=" inline-flex flex-col items-center justify-between px-4 hover:bg-gray-50 group">
